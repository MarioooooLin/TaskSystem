...
OR @CreatedByUserId IS NULL THROW 50001,
N'找不到已通過審核且帳號啟用中的業者，請先執行業者 seed。',
1;
...
WHERE c.MerchantId = @MerchantId
    AND NOT EXISTS (
        SELECT 1
        FROM CaseCategories AS existing
        WHERE existing.CaseId = c.Id
    );
...
WHERE c.MerchantId = @MerchantId
    AND NOT EXISTS (
        SELECT 1
        FROM CaseLanguages AS existing
        WHERE existing.CaseId = c.Id
    );
...
WHERE c.MerchantId = @MerchantId
    AND NOT EXISTS (
        SELECT 1
        FROM CaseRequirements AS existing
        WHERE existing.CaseId = c.Id
    );
...
WHERE c.MerchantId = @MerchantId
    AND NOT EXISTS (
        SELECT 1
        FROM CaseBarterItems AS existing
        WHERE existing.CaseId = c.Id
    );
...
WHERE c.MerchantId = @MerchantId
    AND NOT EXISTS (
        SELECT 1
        FROM CaseBudgetSnapshots AS existing
        WHERE existing.CaseId = c.Id
    );
COMMIT TRANSACTION;
