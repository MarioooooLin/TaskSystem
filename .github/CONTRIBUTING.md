...
- 高風險操作包含匯出、停權/解除停權、代理登入、帳務調整、角色權限異動、審核結果、異議處理。
...
- 折扣金採獨立 `MerchantCreditWallets` / `MerchantCreditTransactions`，不混入現金錢包；所有加值、扣回、折抵、退回、到期與人工調整都必須走折扣金交易流水。折抵規則尚待 PM/財務確認，第一版先保留餘額、加值/扣回、異動紀錄與使用紀錄能力。
...
- 外部設定檔的路徑規範與部署注入方式。
- 帳號註冊所需欄位。
- 業者復權規則：全部成員復權、僅 Owner 復權，或依停用前狀態還原。
- 折扣金折抵規則：可折抵項目、是否影響案件發布時現金鎖款、案件取消是否退回、有效期限與使用順序。

### 9.2 KOL / 審核

- `ADM-005` 匯出 KOL 清單是否包含手機、Email、收款狀態等個資欄位。
- `ADM-006` 多社群平台驗證狀態不一致時，列表與詳情摘要顯示規則。
- `ADM-015` 待審核、重送審核、已退回待補是否需要新增審核流程欄位，或以 `VerificationStatus` 加紀錄推導。
- `ADM-015` 資料完整度計算公式。
- `KOL-001` LINE Login 後 Email 缺漏時，是允許 `Users.Email` nullable，或首次登入強制補填 Email。
- `KOL-005` LINE 通知需依 LINE Messaging API userId 推播；需確認 LINE 綁定與推播失敗 fallback。

### 9.3 案件 / 導購 / 帳務

- `ADM-008` 批次操作可執行哪些動作、可作用於哪些任務狀態。
- `ADM-008` 匯出 KOL 任務清單是否包含 KOL 聯絡方式或成果連結等個資/敏感資料。
- 導購同步異常、訂單統計與預估分潤需依 `ReferralOrders` 或等價資料表補齊，資料來源需確認。
- `ADM-011` 平台總收入、平台總支出、平台毛利的精確公式與預設統計期間。
- `ADM-011` 匯出報表欄位、格式與是否包含 KOL 個人收益明細。

### 9.4 業者端

- `MER-HOME` 待審核案件狀態需重新命名或對齊既有案件狀態；目前程式碼 `CaseStatus` 無 `PendingReview`，MEMORY 也記錄業者不走審核流程。
- `MER-016` 導購訂單交易狀態需確認是否沿用可結算、處理中、異常交易、已取消。
- `MER-016` 匯出導購明細是否包含折扣碼、CID、KOL 個資與分潤金額。
